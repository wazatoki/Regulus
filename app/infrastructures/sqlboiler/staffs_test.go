...
func testStaffToManyQueryConditions(t *testing.T) {
	var err error
	ctx := context.Background()
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()
	...
	_, err = tx.Exec("insert into \"favorite_conditions\" (\"staffs_id\", \"query_conditions_id\") values ($1, $2)", a.ID, b.ID)
	if err != nil {
		t.Fatal(err)
	}
	_, err = tx.Exec("insert into \"favorite_conditions\" (\"staffs_id\", \"query_conditions_id\") values ($1, $2)", a.ID, c.ID)
	if err != nil {
		t.Fatal(err)
	}
	...
	slice := StaffSlice{&a}
	if err = a.L.LoadQueryConditions(ctx, tx, false, (*[]*Staff)(&slice), nil); err != nil {
		t.Fatal(err)
	}
...
func testStaffToManyAddOpQueryConditions(t *testing.T) {
	var err error
...
func testStaffToManySetOpQueryConditions(t *testing.T) {
	var err error
...
func testStaffToManyRemoveOpQueryConditions(t *testing.T) {
	var err error
