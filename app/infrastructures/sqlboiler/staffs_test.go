...
func testStaffToManyFavoriteConditions(t *testing.T) {
	var err error
	ctx := context.Background()
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()
	...
	for _, v := range check {
		if v.StaffsID == b.StaffsID {
			bFound = true
		}
		if v.StaffsID == c.StaffsID {
			cFound = true
		}
	}
	...
	slice := StaffSlice{&a}
	if err = a.L.LoadFavoriteConditions(ctx, tx, false, (*[]*Staff)(&slice), nil); err != nil {
		t.Fatal(err)
	}
...
func testStaffToManyAddOpFavoriteConditions(t *testing.T) {
	var err error
	...
	for i, x := range foreignersSplitByInsertion {
		...
		if a.ID != first.StaffsID {
			t.Error("foreign key was wrong value", a.ID, first.StaffsID)
		}
		if a.ID != second.StaffsID {
			t.Error("foreign key was wrong value", a.ID, second.StaffsID)
		}
